...
type NavItemView = NavItem & {
  accessLabel: string;
};

function getAccessLabel(item: NavItem, context: CurrentUserContext) {
  switch (item.requiredAccess) {
    case "crew":
      return context.canAccessCrew ? "Member" : "Locked";
    case "dla":
      return context.canAccessDla ? "DLA access" : "Locked";
    case "admin":
      return context.canAccessAdmin ? "Admin access" : "Locked";
    default:
      return "Open";
  }
}
...
export function AppShellClient({
  ...
  const roleHeadline =
    context.isSuperAdmin
      ? "Super admin"
      : context.primaryStationRole ?? "Station member";
  const roleDetail =
    context.primaryStationName ?? context.profile?.display_name ?? "Launch Ready";
  const navItems: NavItemView[] = NAV_ITEMS.map((item) => ({
    ...item,
    accessLabel: getAccessLabel(item, context),
  }));
        ...
        <aside className="hidden w-80 shrink-0 border-r border-border/70 bg-slate-950/45 px-5 py-6 lg:flex lg:flex-col">
          ...
          <div className="mb-5 rounded-3xl border border-white/10 bg-white/5 p-4">
            ...
            <p className="mt-2 text-sm font-medium text-card-foreground">
              {context.profile?.display_name ?? context.profile?.email ?? "Crew member"}
            </p>
            <p className="mt-1 text-sm text-muted-foreground">{roleHeadline}</p>
            <p className="mt-1 text-xs text-muted-foreground">{roleDetail}</p>
